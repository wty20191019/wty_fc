...
void OLED_ShowSignedNum(u8 x,u8 y,int32_t num,u8 len,u8 size1,u8 mode)
{
	...
	OLED_ShowNum(x, y, value, len, size1, mode);
}
...
void OLED_ShowHexNum(u8 x,u8 y,u32 num,u8 len,u8 size1,u8 mode)
{
	...
	for (i = 0U; i < len; ++i)
	{
		shift = (u8)((len - i - 1U) * 4U);
		digit = (u8)((num >> shift) & 0x0FU);
		OLED_ShowChar((u8)(x + charWidth * i), y, (digit < 10U) ? (u8)('0' + digit) : (u8)('A' + (digit - 10U)), size1, mode);
	}
}

void OLED_ShowBinNum(u8 x,u8 y,u32 num,u8 len,u8 size1,u8 mode)
{
	u8 charWidth = (size1 == 8U) ? 6U : (u8)(size1 / 2U);
	u8 i;
	u8 bit;
	...
	for (i = 0U; i < len; ++i)
	{
		bit = (u8)((num >> (len - i - 1U)) & 0x01U);
		OLED_ShowChar((u8)(x + charWidth * i), y, (u8)('0' + bit), size1, mode);
	}
}
...
void OLED_ShowFloatNum(u8 x,u8 y,double num,u8 intLen,u8 fraLen,u8 size1,u8 mode)
{
	...
	if (fraLen > 0U)
	{
		OLED_ShowChar(x, y, '.', size1, mode);
		x = (u8)(x + charWidth);
		OLED_ShowNum(x, y, fraPart, fraLen, size1, mode);
	}
}
