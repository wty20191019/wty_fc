...
//  {
//    Total_Controller.High_Position_Control.Expect=NamelessQuad.Location.z;//一直更新高度期望
//  }
//  else
//  {
...
//      Total_Controller.High_Position_Control.FeedBack=NamelessQuad.Location.z;//反馈
...
u8 alt_handmove_check(void)
{
  /*u8 alt_handmove=FALSE;
  alt_handmove=(u8)((RC_NewData[0]<=Deadzone_Min) || (RC_NewData[0]>=Deadzone_Max));
  return alt_handmove;*/
}

u8 pos_handmove_check(void)
{
  //u8 pos_handmove=FALSE;
  //pos_handmove=(u8)(Roll_Control!=0||Pitch_Control!=0);//无水平遥控量给定
  //return pos_handmove;
}

u8 land_poshold(Vector2_Nav target_pos,Vector2_Nav now_pos,uint8_t fixed_state)
{
  ...
  //      {
  //        //位置期望,经纬、航行速度、高度
  //        Total_Controller.Latitude_Position_Control.Expect=NamelessQuad.Location.x;//移植更新水平目标位置，目的是切回定点模式赋悬停点
  //        Total_Controller.Longitude_Position_Control.Expect=NamelessQuad.Location.y;
  //        //导航坐标系下E、N方向上位置偏差
...
//       &&ABS(NamelessQuad.Speed[_YAW])<=20.0//惯导竖直轴速度+-20cm/s
...
u8 check_pos_handmove=FALSE;
u8 check_alt_handmove=FALSE;
...
u8 GPS_ok()// returns true if the GPS is ok and home position is set
{
